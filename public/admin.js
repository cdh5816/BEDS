...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderDetail(site) {
    ...
    detailBodyEl.innerHTML = `
      <div class="grid gap-1">
        ...
        <div><span class="text-slate-400">안전:</span> ${escapeHtml(statusKo(site.status))} (${escapeHtml(site.status || 'SAFE')})</div>
      ...
      const popupHtml = `
        <div style="min-width:240px">
          ...
          <div style="font-size:11px;color:#94a3b8;margin-top:6px">
            ${badge} · ${cko} · 센서 ${Number(s.sensorCount ?? 0)}개
          </div>
        </div>
      `;
  ...
  function renderList() {
    ...
    filtered.forEach((s) => {
      ...
      badge.textContent = (s.status || 'SAFE');
